    ...
    <div className="flex h-full">
      <div className={`fixed inset-y-0 left-0 transform ${isOpen ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0 transition-transform duration-200 ease-in-out bg-primary text-white w-64 z-40`}>
        ...
        <nav className="mt-4 flex flex-col h-3/4 justify-between">
          <ul>
            ...
            <li className="p-2  cursor-pointer">
              <NavLink to="/profile" className={({ isActive }) => `font-roboto flex items-center justify-center w-full p-2 rounded-full hover:bg-secondary hover:text-white  ${isActive ? "bg-secondary text-white" : "text-black bg-white"}`}>
      ...
      {!isOpen && (
        <div className="lg:hidden fixed top-0 left-0 z-50">
          <button className="m-4 focus:outline-none bg-secondary rounded-md p-1" onClick={toggleSidebar}>
